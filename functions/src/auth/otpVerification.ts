...
export const triggerSignupVerification = onCall(
  { region: 'asia-south1' },
  async (request) => {
    ...
    try {
      ...
      if (err.code !== 'auth/user-not-found' && err instanceof HttpsError) throw err;
    }
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to send email verification code.');
    }
...
export const confirmSignup = onCall(
  { region: 'asia-south1' },
  async (request) => {
    const { verificationId, emailCode, whatsappCode } = request.data;

    if (!verificationId || !emailCode || !whatsappCode) {
      throw new HttpsError('invalid-argument', 'Verification ID and both codes are required.');
    }
    ...
    if (!snap.exists) {
      throw new HttpsError('not-found', 'Verification expired or not found. Please try again.');
    }
    ...
    if (new Date() > expiresAt) {
      await docRef.delete();
      throw new HttpsError('deadline-exceeded', 'Verification codes have expired. Please request new codes.');
    }

    // Check failed attempts limit
    const attempts = (data.failedAttempts || 0);
    if (attempts >= 3) {
      throw new HttpsError('resource-exhausted', 'Too many failed attempts. Please contact support at hello@pixalara.com');
    }
    ...
    if (emailCode !== data.emailOTP || whatsappCode !== data.whatsappOTP) {
      ...
      throw new HttpsError('permission-denied', `Invalid code. ${remaining} attempt${remaining === 1 ? '' : 's'} remaining.`);
    }
...
export const resendWhatsAppOTP = onCall(
  { region: 'asia-south1' },
  async (request) => {
    const { verificationId } = request.data;
    if (!verificationId) throw new HttpsError('invalid-argument', 'Verification ID required.');

    const docRef = db.collection('pendingVerifications').doc(verificationId);
    const snap = await docRef.get();

    if (!snap.exists) {
      throw new HttpsError('not-found', 'Verification expired. Please start over.');
    }
...
export const sendWhatsAppOnlyOTP = onCall(
  { region: 'asia-south1' },
  async (request) => {
...
export const confirmWhatsAppOTP = onCall(
  { region: 'asia-south1' },
  async (request) => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'Login required.');

    const { verificationId, code } = request.data;
    if (!verificationId || !code) throw new HttpsError('invalid-argument', 'Verification ID and code required.');

    const docRef = db.collection('pendingVerifications').doc(verificationId);
    const snap = await docRef.get();

    if (!snap.exists) throw new HttpsError('not-found', 'Verification expired. Please try again.');

    const data = snap.data()!;

    // Verify ownership
    if (data.uid !== request.auth.uid) throw new HttpsError('permission-denied', 'Invalid verification.');
    ...
    const attempts = data.failedAttempts || 0;
    if (attempts >= 3) {
      throw new HttpsError('resource-exhausted', 'Too many failed attempts. Contact hello@pixalara.com');
    }
    ...
    if (code !== data.whatsappOTP) {
      await docRef.update({ failedAttempts: attempts + 1 });
      const remaining = 2 - attempts;
      throw new HttpsError('permission-denied', remaining <= 0
