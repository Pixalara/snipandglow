...
export async function createInvoice(
  ...
  if (finalDiscount > 0) {
    ...
    const { data: cm } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    customerMembershipId = (cm as { id: string } | null)?.id ?? null;
  }
  ...
  // Attribute the bill to the membership. Separate from the insert because the
  // column is absent from the (stale) generated types; cast is isolated here so
  // the insert above stays fully type-checked.
